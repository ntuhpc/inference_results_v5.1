...
## Set up dataset, model, and logging directories
...
# export CODE_DIR=$PWD # For development
```

## Build the docker (development)
```
# XPU
bash docker/build_whl.sh # Build
bash docker/build_container.sh 
# CPU
# bash docker/build_container_cpu.sh
```
...
```
export DOCKER_IMAGE=vllm_xpu:test_u24 
# export DOCKER_IMAGE=vllm_cpu:test_u24
# bash docker/run_container_cpu.sh # For CPU
...
## Run Benchmark (development)
Initialize environment:
```
source init_env.sh {llama2-70b llama3_1-8b} {offline server} # server not validated for 8b
```
Calibrate model (if haven't already):
```
