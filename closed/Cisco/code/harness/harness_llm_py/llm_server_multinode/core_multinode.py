...
class LLMCore():
    ...
    def __init__(
        ...
        TP = int(harness_config.trtllm_build_flags['tensor_parallelism'])
        PP = int(harness_config.trtllm_build_flags['pipeline_parallelism'])
        global num_cores, group_size
        group_size = TP * PP
        num_cores = size // group_size
        ...
        self.send_stats = send_stats_callback
        self.print_stats_thresh = 1000
        ...
        print(f"Rank {rank} initializing LLMCore: {self.name}")
        ...
        print(f"Rank {rank} initialized LLMCore: {self.name}")
    ...
    def initialize_response_thread(self):
        ...
        """
        self.response_thread = threading.Thread(target=self.poll_responses, args=(rank,))
    ...
    def poll_responses(self, rank):
        ...
        while True:
            ...
            if self.executor.can_enqueue_requests() and (stats := self.executor.get_latest_iteration_stats()):
                ...
                self.send_stats(additional_unit_updates)
        ...
        print(f"Core {self.name} response thread completed, processed {self.processed_count} samples.")
    ...
    def flush(self):
        ...
        with self.nvtx_scope("flush"):
            if not self.response_thread_exit.is_set():
                with self.flush_signal:
                    print(f"Rank {rank} waiting for response thread to complete.")
                    self.flush_signal.wait()
                    print(f"Rank {rank} response thread completed.")
