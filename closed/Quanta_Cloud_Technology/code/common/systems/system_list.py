...
add_systems("H200_SXM_141GB_CTSx{}",
            ...
            tags=["start_from_device_enabled"])
add_systems("H200_NVL_141GBx{}",
            "D75E-4U_H200-NVL-141GBx{}",
            KnownCPU.x86_64_Intel_Generic,
            KnownGPU.H200_NVL,
            [1,2,4,8],
            Memory(100, ByteSuffix.GiB))
...
def numa_config_string(L: List[NUMANode]):
    ...
    def _node_str(n):
        # Add by QCT, for D75E NUMA topology
        if GPU not in n.accelerators:
            return None  # Skip this NUMA node
    ...
    # Modify by QCT, for D75E NUMA topology
    return '&'.join(
            s for s in (_node_str(n) for n in nodes) if s is not None
    )
