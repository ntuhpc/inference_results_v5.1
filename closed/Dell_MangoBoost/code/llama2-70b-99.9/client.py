...
flags.DEFINE_integer("qps", 128, "Number of parallel requests")
...
flags.DEFINE_list(
    "sut_server_addr", ["http://localhost"], "List of server addresses"
)
flags.DEFINE_integer("port", 8000, "Servers port, where all the ports should use the same port number")
flags.DEFINE_string(
    "scheduler",
    "round_robin",
    "The scheduler algorithm for scheduling the requests among nodes.",
)
flags.DEFINE_list(
    "scheduler_weights",
    [],
    "The scheduling weights of each node. Only works when the scheduler is weighted_random.",
)
flags.DEFINE_bool(
    "client_send_batch_requests",
    False,
    "Client send the requests to each node in a batching way during the Server Scenario",
)
flags.DEFINE_bool(
