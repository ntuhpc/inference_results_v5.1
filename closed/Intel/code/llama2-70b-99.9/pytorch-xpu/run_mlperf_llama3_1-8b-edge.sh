...
prepare_suplements () {
  ...
  cp -r README.llama3.1-8B.md ${CODE_PATH}/

  # Populate /logs/measurements directory
  cp measurements.json ${MEASUREMENTS_PATH}/${SYSTEM}.json
  cp README.llama3.1-8B.md user.conf calibration/quantize_8b.sh ${MEASUREMENTS_PATH}/
...
source configure_workload_llama3_1-8b-edge.sh
