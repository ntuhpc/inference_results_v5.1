...
if [[ "$PREPARE_DATA" == "1" ]]; then
    ...
    docker exec ${LAB_DKR_CTNAME} bash /lab-mlperf-inference/setup/download_llama2_70b.sh

    docker exec ${LAB_DKR_CTNAME} bash /lab-mlperf-inference/setup/quantize_llama2_70b.sh

    docker stop ${LAB_DKR_CTNAME}
fi

MLPERF_IMAGE_NAME=rocm/mlperf-inference:submission_5.1-llama2_70b

if ! docker image inspect "${MLPERF_IMAGE_NAME}" > /dev/null 2>&1; then
    bash $MLPINF_DIR/setup/build_submission_llama2_70b.sh $MLPERF_IMAGE_NAME
fi
...
docker exec --env COMPANY="TEST" \
            ...
            ${LAB_DKR_CTNAME} bash /lab-mlperf-inference/submission/llama2_70b.sh
