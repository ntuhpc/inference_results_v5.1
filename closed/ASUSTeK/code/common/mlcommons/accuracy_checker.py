...
class DeepSeek_R1AccuracyChecker(AccuracyChecker):
    ...
    def get_accuracy(self) -> List[Dict[Any, Any]]:
        """Runs the accuracy script and get_accuracys the accuracy results for DeepSeek-R1.
        """
        output = self.run()
        full_output = "\n".join(output)

        # Extract the JSON results block
        result_match = re.search(r"Evaluation Results:\s*(\{.*?\})", full_output, re.DOTALL)
        if not result_match:
            logging.error("Could not find 'Evaluation Results:' block in the accuracy script output.")
            # Fallback to writing full output
            accuracy_output_to_write = full_output
            parsed_json = {}
        else:
            # Reconstruct the desired output format
            parsed_json = json.loads(result_match.group(1))
            accuracy_output_to_write = "Evaluation Results: " + json.dumps(parsed_json, indent=2)

        # Write filtered output to accuracy.txt
        with open(os.path.join(os.path.dirname(self.log_file), "accuracy.txt"), "w", encoding="utf-8") as f:
            f.write(accuracy_output_to_write)

        # Get results from the parsed JSON
        accuracy_result_list = []
        for i, metric_name in enumerate(self.acc_metric_list):
            threshold = self.threshold_list[i]
            accuracy = parsed_json.get(metric_name)
