...
class AuditTest01Verifier(AuditVerifier):
    def __init__(self, harness):
        super().__init__(AuditTest.TEST01, os.path.join(paths.MLCOMMONS_INF_REPO_PATH, "compliance", "nvidia", "TEST01",
                                                        "run_verification.py"))
        self.harness = harness

    def build_args(self) -> str:
        results_path = os.path.join(paths.RESULTS_STAGING_PATH,
...
class AuditTest04Verifier(AuditVerifier):
    def __init__(self, harness):
        super().__init__(AuditTest.TEST04, os.path.join(paths.MLCOMMONS_INF_REPO_PATH, "compliance", "nvidia", "TEST04",
                                                        "run_verification.py"))
        self.harness = harness
...
class AuditTest06Verifier(AuditVerifier):
    def __init__(self, harness):
        super().__init__(AuditTest.TEST06, os.path.join(paths.MLCOMMONS_INF_REPO_PATH, "compliance", "nvidia", "TEST06",
                                                        "run_verification.py"))
        self.harness = harness
