...
def main():
    ...
    model = model.to(torch.bfloat16)  # Convert model to float16 for quantization
    #model.seqlen = 3072
    ...
    export_dir = output_dir + "/" + model_name.split('/')[-1] + f"-autoround-w{bits}g{group_size}-iters{iters}-cpu"
