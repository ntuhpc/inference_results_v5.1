# Cisco MLPerf Inference v5.1 — Quick-Start Guide

---

## 1 · Pull & Enter the NVIDIA Container
1. Follow NVIDIA’s official *setup* directions (pasted below for convenience).  
2. Run **`start_container.sh`**.  
   * Edit the script so **`<container-name>`** matches the image you built with `make prebuild`.
3. (Optional but recommended) **Build engines now** and `docker commit` the container.  
   * If you skip this, you’ll have to rebuild engines every run.  
   * *Note:* Llama-3.1-405B stores KV-cache files in **`/tmp`**; commit them or you’ll hit a “file not found” error later.

---

## 2 · Configure `environment_var_resolver.sh`
Update the variables below so they point at the right datasets, models, engines, etc.

| Variable | What it should point to |
|----------|-------------------------|
| **`TENSOR_PATH`**        | Pre-processed dataset directory (70B / 405B) |
| **`HUGGINGFACE_CHECKPOINT`** | Model checkpoint directory |
| **`DATASET_PATH`**       | Raw `*.pkl` dataset |
| **`HOSTFILE`**           | MPI hostfile (last node needs **one extra slot** for LoadGen) |
| **`CONT_BASE_NAME`**     | Image name used by remote `docker run` |
| **`ENGINE_PATH`**        | Directory that holds the TRT-LLM engine |
| **`BATCH_SIZE`**         | Engine batch size |
| **`MLPERF_CONF_PATH`** / **`USER_CONF_PATH`** | Corresponding `*.conf` files |
| **`LOG_OUTPUT`**         | Where test logs are written |
| **`TRTLLM_BUILD_FLAGS`** | Copy from the `make run_harness` output |
| **`TRTLLM_RUNTIME_FLAGS`** | Copy from the `make run_harness` output |

---

## 3 · Edit `run_multinode_inference.sh`
Tweak the high-level knobs:

```bash
NUM_GPUS   # total GPUs across all nodes
SCENARIO   # Offline | Server
TEST_MODE  # PerformanceOnly | AccuracyOnly | SubmissionRun
MODEL      # llama3_1-405b | llama2-70b
GPU_TYPE   # H200 | H100
```
...
```bash
bash run_multinode_inference.sh
```

That’s it. The job should start now. The logs land in ${LOG_OUTPUT}, engines stay cached inside your committed container, and the extra slot in HOSTFILE runs LoadGen on its own rank.
