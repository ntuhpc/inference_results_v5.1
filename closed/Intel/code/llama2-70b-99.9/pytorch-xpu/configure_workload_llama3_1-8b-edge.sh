...
export WORKLOAD="llama3.1-8b-edge"
export MODEL="llama3_1-8b-edge"
...
workload_specific_run () {
  ...
  if [ "$SCENARIO" == "SingleStream" ]; then
      source init_env.sh llama3_1-8b-edge singlestream
      ...
      source init_env.sh llama3_1-8b-edge offline
      if [ "${MODE}" == "Accuracy" ]; then
          echo "Run ${MODEL} (${SCENARIO} Accuracy)."
          bash run_local.sh accuracy
      else
