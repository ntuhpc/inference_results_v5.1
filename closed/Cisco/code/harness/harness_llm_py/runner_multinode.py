...
from code.harness.harness_llm_py import LLMSUTMultinode, create_qsl_cls_mn
...
def create_qsl(args):
    qsl_cls = create_qsl_cls_mn(args.dataset_cls)
...
def create_sut(args, dataset):
    sut = LLMSUTMultinode(scenario=args.scenario,
                 dataset=dataset,
...
def run_test(args):
    ...
    if rank == size-1:
        ...
        # start once servers are warmed up
        comm.Barrier()
        ...
        server.warm_up()
        # send last rank a signal that it is warmed up
        comm.Barrier()

    comm.Barrier()  # Ensure all ranks finish before exiting

    if rank == size-1:
        logging.info("Run completed successfully.")
        del sut
        del qsl
        logging.info("SUT and QSL destroyed.")
    else:
        server.stop_work()
        del server
...
if __name__ == '__main__':
    ...
    for thread in threading.enumerate():
        print(f"Thread: {thread.name}, ID: {thread.ident}, Daemon: {thread.daemon}")
    
    # end all threads
    for thread in threading.enumerate():
        if thread is not threading.main_thread():
            print(f"Ending thread: {thread.name}, ID: {thread.ident}")
