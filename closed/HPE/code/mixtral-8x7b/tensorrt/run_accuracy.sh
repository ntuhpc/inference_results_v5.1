...
CMD="python3.9 /work/$(basename $TMP_DIR)/$(basename $MODULE_PATH) --checkpoint-path 'mistralai/Mixtral-8x7B-Instruct-v0.1' --mlperf-accuracy-file /work/$(basename $TMP_DIR)/$(basename $MLPERF_ACCURACY_FILE) --dataset-file $DATASET_FILE --dtype int32 --n_workers 2 --verbose"

sudo docker build -q -t $IMG_NAME --network host - < /work/code/mixtral-8x7b/tensorrt/Dockerfile.accuracy
CONTAINER=$(sudo -E docker run -d -it -w /work -v ${MLPERF_SCRATCH_PATH}:${MLPERF_SCRATCH_PATH} -e HUGGING_FACE_HUB_TOKEN=${HUGGING_FACE_HUB_TOKEN} -v $DATASET_FILE:$DATASET_FILE $IMG_NAME)
